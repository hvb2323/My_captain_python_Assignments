...
def load_data():
    # Load existing data from the file or initialize a new data structure
    if os.path.exists(DATA_FILE):
        with open(DATA_FILE, "r") as file:
            return json.load(file)
    else:
        # Default data structure with predefined categories
...
def add_expense(data):
    # Allow the user to input details for a new expense, including category selection
...
def view_category_summaries(data):
    # Display summaries for each expense category
    if not data["expenses"]:
        print("No expenses recorded.")
        return
...
def remove_category(data):
    # Allow the user to remove an existing category
    print("\nExisting Categories:")
    for i, category in enumerate(data["categories"], start=1):
        print(f"{i}. {category}")
